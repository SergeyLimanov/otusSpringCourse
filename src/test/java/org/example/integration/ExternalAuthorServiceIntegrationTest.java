...
class ExternalAuthorServiceIntegrationTest {
    ...
    @Test
    void testActuator_CircuitBreakerHealth() {
        // Act
        ...
        assertTrue(response.getBody().contains("circuitBreakers") || 
                ...
                "Health endpoint должен содержать информацию о Circuit Breakers");
    }

    @Test
    void testActuator_CircuitBreakers() {
        // Act
        ResponseEntity<String> response = restTemplate.getForEntity(
                "http://localhost:" + port + "/actuator/circuitbreakers",
                String.class
        );
        ...
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertNotNull(response.getBody());
        assertTrue(response.getBody().contains("externalAuthors"),
                "Должен быть Circuit Breaker с именем 'externalAuthors'");
    }

    @Test
    void testActuator_CircuitBreakerEvents() {
        // Arrange - делаем вызов для генерации событий
        externalAuthorService.getSouthAmericaAuthors(null);

        // Act
        ResponseEntity<String> response = restTemplate.getForEntity(
                "http://localhost:" + port + "/actuator/circuitbreakerevents/externalAuthors",
                String.class
        );
    ...
    @Test
    void testActuator_Retries() {
        // Act
        ResponseEntity<String> response = restTemplate.getForEntity(
                "http://localhost:" + port + "/actuator/retries",
                String.class
        );
        ...
        assertNotNull(response.getBody());
        assertTrue(response.getBody().contains("externalAuthors"),
                "Должен быть Retry с именем 'externalAuthors'");
    }

    @Test
    void testActuator_RetryEvents() {
        // Arrange - делаем вызов для генерации событий
        externalAuthorService.getSouthAmericaAuthors(null);

        // Act
        ResponseEntity<String> response = restTemplate.getForEntity(
                "http://localhost:" + port + "/actuator/retryevents/externalAuthors",
                String.class
        );
